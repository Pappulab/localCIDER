...
DEFAULT_VALS = {'BIN_MIN':0.05,'BIN_MAX':0.95,'NUMBER_OF_BINS':20,'FLATCHECK_FREQ':10000,'CONVERGENCE':np.exp(0.0001), 'FLATNESS_CRITERION': 0.7, 'WL_TYPE':'ZOOM'}

# list of expected keywords
KEYWORD_LIST = ['SEQFILE', 'OUTDIR', 'FREEZE_FILE', 'BIN_MIN','BIN_MAX','NUMBER_OF_BINS', 'FLATCHECK_FREQ', 'CONVERGENCE', 'FLATNESS_CRITERION', 'WL_TYPE']
...
class KeyFile():
    ...
    def parse_keyfile(self, filename):
        ...
        for line in content:
            ...
            if line_list[0].strip() in self.KEYWORDS:
                ...
                # there was more than one whitespace seperated string after the keyword - we basically fail at this
                else:
                    raise KeyFileException("Error: Found keyword " + str(line_list[0].strip()) + " but unable to parse associated value")
            else:
                warning_message("Found unexpected keyword [" + str(line_list[0].strip()) + "] - ignorning...")

        # Now add default for the sequene, which will hopefully be set in the next section by parsing the sequencefile
        ...
        for keyword in KEYWORD_LIST:
            ...
            if keyword == "SEQFILE":
                if value == "":
                    raise KeyFileException("ERROR: No sequence file provided in keyfile (expecting keyword [SEQFILE])")
                else: 
                    if not os.path.isfile(value):
                        raise KeyFileException("Expected " + str(value) + " to be file")

                    # if its a file lets try and extract a sequence from it!                    
                    self.KEYWORDS["SEQUENCE"] = SeqFileParser.parseSeqFile(value)

                    # if we get here we *should* now have a sequence...
                    if self.KEYWORDS["SEQUENCE"] == "" or self.KEYWORDS["SEQUENCE"] is None:
                        raise KeyFileException("ERROR: No sequence was parsed from the sequence file...")
            ...
            ##                                                             
                ...
                if value == "":
                    raise KeyFileException("ERROR: No output directory provided in keyfile (expecting keyword [OUTDIR]")
                else:
                    # creates the output directory if it doesn't already exist
                    if not os.path.exists(value):
                        status_message("Creating output directory " + str(value))
                        try:
                            os.makedirs(value)
                        except OSError, e:
                            print "----------------------------"
                    ...
                    # if it does exist raise a quick warning
                        ...
                        if len(os.listdir(value)) > 0:
                            warning_message("Output directory exists already and is not empty [RISK OF OVERWRITING!]")
                        else:
            ...
            ##
                ...
                if value == "":
                    ...
                    status_message("Setting WL type to default [" + str(DEFAULT_VALS[keyword]) +"]")
                else:
                    if value in WL_TYPES:
                        self.KEYWORDS[keyword] = value
                        status_message("Setting WL type to keyfile defined [" + str(value) +"]")
                    else:
                        raise KeyFileException("Unexpected WL algorithm type selected " + str(value) + " ")
    ...
    def __set_numeric(self, keyword, value):
        ...
        """
        if value == "":
            status_message("Setting " + keyword + " to default [" + str(DEFAULT_VALS[keyword]) + "]")
            self.KEYWORDS[keyword] = DEFAULT_VALS[keyword]
        else:
            try:
                float(value)              
                status_message("Setting " + keyword + " to keyfile defined [" + str(value) + "]")
                self.KEYWORDS[keyword] = value
            except ValueError:
