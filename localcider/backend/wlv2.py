# -*- coding: utf-8 -*-
"""
Created on Fri May 09 14:01:06 2014
...
class WangLandauMachine:
    def __init__(self, seq, writedir, frzfile='', nbins=10, binmin=0, binmax=1, flatchk=10000, flatcrit = .7,convergence=np.exp(.000001), WL_type="ZOOM"):

        # the self.seq must be a sequence object
        if seq.__class__.__name__ == "Sequence":
            self.seq = seq                     # Input was already a sequence object
        else:            
            self.seq = Sequence(seq)           # Convert a string into a sequence object
        
        

        # set some general variables
        self.writeDir = writedir               # Writable output dir
        self.nflatchk = int(flatchk)           # Frequency at which a flatcheck is performed
        ...
        # do the same for the non-adaptive WL histogram
            ...
            self.relevant_min = np.argmin(abs(bincts-(self.binmin+(binWidth/2))))
        ...
        with open(frzfile) as f:
            ...
            for line in f:
                ...
                if columns[0] > columns[1]:
                    raise SequenceError("FREEZEFILE ERROR: First index position in freeze file is greater than second (" + str(line) +")")
    ...
    def run(self):
        ...
        if self.WL_type == "ZOOM":
            return self.run_histogramZoomWL()


    def run_normal_WL(self, zoomcount = 1):
        """
    ...
    def __run_flatcheck(self, H, Hlocal, niter, f, hlog, glog, g):
        ...
        flatness_number=len(np.where(Hlocal/np.mean(Hlocal) >= self.flatcrit)[0])

        print("Number of histograms which meet flat check criterion: " + str(flatness_number))
        ...
        if flatness_number == self.nbins_target:
            ...
            print("f = " + str(f) + " convergence set at " + str(self.convergence))
            self.writeLog(glog,str(niter)+"\t"+self.fprintGVector(g)+"\n")
    ...
    def histogram_zoom(self,Hlocal,H,zoomcount):
        ...
        # histogram ("pushing against the maximimum or pushing against the minimum)
        ...
        full_histogram_new_end = self.relevant_max-(self.nbins_target-new_end)
        ...
        # and max values
        new_start_kappa = ((new_min_kappa - old_start_kappa)/2) + old_start_kappa
    ...
    def __correct_for_numerical_errors(self, new_start_kappa, new_end_kappa, pushing_min, pushing_max,bin_centers):
        ...
        if len(bin_centers[self.relevant_min:self.relevant_max+1]) > self.nbins_target:
            ...
            if pushing_max:
                ...
                # if the lower kappa bin deviates from the ideal kappa min the most
